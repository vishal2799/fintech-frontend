...
export default function EmployeeFormPage() {
  ...
  useEffect(() => {
    if (isEdit && employees.length > 0) {
      ...
      if (match) setInitialValues({ ...match, roleId: match.role?.id });
    }
  }, [isEdit, employees, id]);
  ...
  return (
    <Container size="lg">
      <Title order={2} mb="md">{isEdit ? 'Edit Employee' : 'Create Employee'}</Title>
      <EmployeeForm mode={isEdit ? 'edit' : 'create'} initialValues={initialValues || {}} />
    </Container>
  );
}
